...
export class TransitionScroll extends Transition {
	...
	constructor(size: number, lis: HTMLLIElement[], tranTime: number) {
		...
		const ps: number[] = this._calcPosition(0, 1);
	...
	async transition(idx: number, dir: number): Promise<void> {
		this._queue.push((cur: number, curPsd: number) => this._doTransition(cur, curPsd, idx, dir));
	...
	async _doTransition(curIdx: number, curIdxPsd: number, idx: number, dir: number): Promise<[number, number]> {
		...
		let ps: number[] = this._calcPosition(curIdxPsd, dir);
		for (let i: number = 0; i < this._lis.length; i += 1) {
	...
	_shift(curPs: number[], curIdxPsd: number, dir: number, time: number, tf = 'ease'): [number[], number] {
		...
		const ps: number[] = this._calcPosition(idxPsd, dir);
